...
new_rf <- train(Species ~ ., iris, method='rf', 
...
control <- trainControl(method='cv', number=5)
dt <- train(Species ~ ., iris, method='rpart', 
            metric='Accuracy', trControl=control)
rf <- train(Species ~ ., iris, method='rf', 
            metric='Accuracy', trControl=control)
sv <- train(Species ~ ., iris, method='svmRadial', 
            metric='Accuracy', trControl=control)
kn <- train(Species ~ ., iris, method='knn', 
            metric='Accuracy', trControl=control)
resamp <- resamples(list(결정트리 = dt, 랜덤포레스트 = rf, SVM = sv, kNN = kn))
summary(resamp)

sort(resamp, decreasing = T)      # 정확도(accuracy) 높은 순
dotplot(resamp)
