...
head(iris)
dtc <- rpart(Species ~ ., data=iris)   # iris 데이터를 결정 트리로 학습
summary(dtc)
dtc
...
# 평가
library(caret)
confusionMatrix(pred, iris$Species)    # Accuracy 항목
...
# 모델링
dtc <- rpart(Species ~ ., data=iris_train)

# 예측
pred <- predict(dtc, data=iris_test, type='class')
pred
