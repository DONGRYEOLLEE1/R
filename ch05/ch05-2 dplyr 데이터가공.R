...
arrange(europe_pop, desc(lifeExp))          # 내림차순 (desc)

# 3-1. 아프리카 대륙에서 평균 수명이 가장 긴 나라 5곳
...
gapminder %>% filter(continent=="Africa" & year==2007) %>% arrange(desc(lifeExp)) %>% head(5)

# 4. group_by 와 summarize
summarize(africa_pop, pop_avg=mean(pop))                       # 2007년 아프리카 국가별 평균 수명
summarize(group_by(gapminder, continent), pop_avg=mean(pop))   # 대륙별 인구평균
...
summarize(group_by(asia_pop, country), life_avg=mean(lifeExp)) %>% arrange(desc(life_avg)) %>% head(5)
...
# 2007년 인구수가 5000만 이상인 국가중 기대수명이 가장 큰 5개나라
gapminder %>%
    filter(pop>=50000000 & year==2007) %>%
    group_by(country) %>%
    summarize(life_avg=mean(lifeExp)) %>%
    arrange(desc(life_avg)) %>%
    head(5)
...
# 평균연비가 30이상이면 A 등급, 20이상이면 B 등급, 아니면 C등급
