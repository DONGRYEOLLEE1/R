...
leaflet(map) %>% 
    setView(lng=127.30, lat = 36.35, zoom=12) %>% 
    addProviderTiles('Stamen.TonerLite') %>% 
    addPolygons()
class(map)             #SpatialPolygonsDataFrame
slotNames(map)
map@data
city <- read.csv('Map/대전자치단체.csv', fileEncoding = 'UTF-8')[-1,]

# 연속적인 값의 팔레트
pal <- colorNumeric('RdPu', NULL)
leaflet(map) %>% 
    setView(lng=127.30, lat = 36.35, zoom=11) %>% 
    addProviderTiles('Stamen.TonerLite') %>% 
...
city$name <- lapply(city$place, 
                    function(x) substring(x, 1, str_length(x)-1))  # apply 계열 함수 숙지
...
leaflet(map) %>% 
    ...
    addPolygons(fillColor = ~pal(city$pop), 
                ...
                dashArray = '3', 
                fillOpacity = 0.7, 
                label = ~city$name) #city의 name과 map의 name을 일치시켜줘야함(순서)


# map@data 와 city를 merge해서 하나의 데이터로 처리
...
leaflet(map) %>% 
    ...
    addPolygons(fillColor = ~pal(pop), 
                ...
                dashArray = '3', 
                fillOpacity = 0.7, 
                highlight = highlightOptions(weight = 5, color = '#777', 
                                             dashArray = '', fillColor = 0.7, 
                                             bringToFront = T),
                label = ~name) %>% 
