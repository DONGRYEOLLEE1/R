...
train_list <- createDataPartition(y=movie_review$sentiment, p=0.8, list=F)
mtrain <- movie_review[train_list, ]
mtest <- movie_review[-train_list, ]

# 훈련 데이터셋에 대해 DTM 구축     ( 전처리는 필수)
# 테스트 데이터셋에 대해서도 동일하게 적용해야 함  
doc <- Corpus(VectorSource(mtrain$review))
doc <- tm_map(doc, content_transformer(tolower))
doc <- tm_map(doc, removeNumbers)
stop_words <- c(stopwords('en'), '<bn />')
doc <- tm_map(doc, removeWords, stop_words)
doc <- tm_map(doc, removePunctuation)
doc <- tm_map(doc, stripWhitespace)

dtm <- DocumentTermMatrix(doc, control=list(weighting=weightTf))   # 숫자로 변환
dim(dtm)
inspect(dtm)
...
# 테스트 데이터셋으로 모델 성능 평가         ( 마찬가지로 테스트 셋도 전처리 필)
