women
plot(women)
str(cars)

# 두 줄을 선택한 후 상단에 있는 'Run' 버튼을 누르면 한꺼번에 실행됨
a <- 2
b <- a*a

# 작업 디렉토리 지정
getwd()
setwd("/workspace/R")
getwd(
getwd()
)
getwd()
getwd(
  getwd()
)
getwd()
setwd('/workspace/R')
getwd(
  getwd()
)
getwd()
setwd('/workspace/R')
getwd(
  
  
library(dplyr)
library(dplyr)
library(ggplot2)
library(ggplot2)
search(
search()
)
search()
library(dplyr)
library(ggplot2)
library(ggplot2)
install.packages("ggplot2")
search(
  search()
)
search()
str(iris)
str(iris)# 
head(iris, 10)   # Default는 6
iris
tail(iris)
tail(iris)     # 끝에서 6개를 보여줌
plot(iris)

# 두 속성의 상관관계
plot(iris$petal.length, iris$Petal.Width, col=iris$Species)
plot(iris$Petal.Length, iris$Petal.Width, col=iris$Species)
plot(iris$Petal.Length, iris$Petal.Width, col=iris$Species, pch=18)
plot(iris$Petal.Length, iris$Petal.Width, col=iris$Species, pch=18, main = "iris")
plot(iris$Petal.Length, iris$Petal.Width, col=iris$Species, pch=18, main = "IRIS"
     
     
     # tips.csv download
tips = read.csv(https://raw.githubsercontent.com/mwaskom/seaborn-data/blob/master/tips.csv)
tips = read.csv(https://raw.githubusercontent.com/mwaskom/seaborn-data/blob/master/tips.csv)
tips = read.csv(https://raw.githubusercontent.com/mwaskom/seaborn-data/blob/master/tips.csv)
tips = read.csv(https:/raw.githubusercontent.com/mwaskom/seaborn-data/blob/master/tips.csv)
tips = read.csv(https://raw.githubusercontent.com/mwaskom/seaborn-data/master/tips.csv)
tips=read.csv(https://raw.githubusercontent.com/mwaskom/seaborn-data/master/tips.csv)
tips=read.csv('https://raw.githubusercontent.com/mwaskom/seaborn-data/master/tips.csv')

head(tips)
str(tips)
# 요약 통계
summary(tips)
summary(cars93)
summary(Cars93)


# gglot2 그림 그리기
tips %>% ggplot(aes(size))+geom_histogram()
tips %>% ggplot(aes(size))+geom_histogram()
tips %>% ggplot(aes(totla_bill, tip))+geom_point()
library(ggplot2)
library(dplyr)

tips%>%ggplot(aes(size))+geom_histogram()
tips%>%ggplot(aes(total_bill, tip))+geom_histogram()
tips %>% ggplot(aes(total_bill, tip))+geom_point(aes(col=day))
tips %>% ggplot(aes(total_bill, tip))+geom_point(aes(col=day, pch=sex), size=3)
tips %>% ggplot(aes(total_bill, tip))+geom_point(aes(col=day, pch=time), size=3)
tips %>% ggplot(aes(total_bill, tip))+geom_point(aes(col=day, pch=smoker), size=6)
tips %>% ggplot(aes(size))+geom_histogram()
tips %>% ggplot(aes(total_bill, tip))+geom_point()
tips %>% ggplot(aes(total_bill, tip))+geom_point(size=6)
